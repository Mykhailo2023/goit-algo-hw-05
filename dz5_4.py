...
# декоратор для обробки помилок введення користувачем
def input_error(func):
    def inner(*args, **kwargs):
        function_name = str(func).split(" ")[1]
        if len(args) != 2:  # Перевірка кількості аргументів
            print(f"{Fore.RED}Неправильна кількість аргументів. Потрібно ввести ім'я та номер телефону.{Fore.RESET}")
            return False
        name, phone = args
        if not phone.isdigit():  # Перевірка, чи номер є послідовністю цифр
            print(f"{Fore.LIGHTRED_EX}Номер не є цифрами.{Fore.RESET}")
            return False
        try:
